...
- A plain DNS `A` record for `minecraft.def4alt.com` that points at the Tailscale IP for the Minecraft host.
...
- If you change the Minecraft host or Tailscale target later, update the `minecraft_tailnet_ip` value in `locals.tf` and re-run OpenTofu so the DNS record stays correct.
