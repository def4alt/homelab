...
Pinchflat's `/config/extras/cookies.txt` now contains a plaintext Netscape-format export derived from Helium's encrypted cookie store for the `You` profile. The deployed file is owned by UID/GID 1000 with mode 0600, no cookie values were written to Git or terminal output, and an authenticated yt-dlp subscriptions-feed check passed. Cookies are runtime credentials and will eventually expire; they must be re-exported after the Helium Google session changes. The only remaining user choice is which channels or playlists to add, whether each source should use cookies for all operations, and what per-source historical cutoff or retention to use.
