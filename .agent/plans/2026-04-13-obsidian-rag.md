...
- [x] (2026-04-13 03:40Z) Removed the ripgrep fallback and binary bootstrap after confirming the semantic path is sufficient for the current vault.
...
- Observation: The base `python:3.12-slim` image does not include `rg`.
  Evidence: the live pod returned `sh: 1: rg: not found`, which is why the fallback was temporarily explored and then removed.
...
- Decision: Remove the ripgrep fallback and keep Obsidian search semantic-only.
  Rationale: the semantic search path is now working reliably, and removing the extra binary/bootstrap logic keeps the pod lighter and simpler.
  Date/Author: 2026-04-13 / Codex
